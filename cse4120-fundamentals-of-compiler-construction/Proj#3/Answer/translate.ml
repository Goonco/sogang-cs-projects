...
let two_exp exp1 exp2 symbolMap binOp lhsReg =
  let (il1, opr1) = trans_exp exp1 symbolMap in
  let (il2, opr2) = trans_exp exp2 symbolMap in
  (il1 @ il2 @ [BinOp (lhsReg, binOp, opr1, opr2)])

let one_exp expr symbolMap unOp lhsReg =
  let (il, opr) = trans_exp expr symbolMap in
  (il @ [UnOp (lhsReg, unOp, opr)])
...
let rec trans_stmt stmt symbolMap  =  
  ...
  | LocalDecl (ctyp, vName) -> (
    ...
    match ctyp with
    | CInt | CBool -> (nextSM, [])
    | CIntArr n | CBoolArr n -> (nextSM, [LocalAlloc(r, sizeof ctyp)])
  )
  | Assign (lhsName, exp) -> (
    ...
    | LVar lVarName -> (
      ...
      match exp with
      | Add (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap AddOp lhsReg))
      | Sub (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap SubOp lhsReg))
      | Mul (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap MulOp lhsReg))
      | Div (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap DivOp lhsReg))
      | Equal (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap EqOp lhsReg))
      | NotEq (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap NeqOp lhsReg))
      | LessEq (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap LeqOp lhsReg))
      | LessThan (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap LtOp lhsReg))
      | GreaterEq (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap GeqOp lhsReg))
      | GreaterThan (expr1, expr2) -> (symbolMap, instrs @ (two_exp expr1 expr2 symbolMap GtOp lhsReg))
      | Not expr -> (symbolMap, instrs @ (one_exp expr symbolMap NotOp lhsReg))
      | Neg expr -> (symbolMap, instrs @ (one_exp expr symbolMap NegOp lhsReg))
      | _ -> (
        match rhsOpr with
        | Imm im -> (symbolMap, instrs @ [Set (lhsReg, im)])
        | Reg rg -> (symbolMap, instrs @ [Copy (rg, lhsReg)])
      )
    )
    | LArr (varName, exp) -> (
      let (movedInst, movedReg) = calculateArr symbolMap varName exp in
  ...
  | If (condExp, true_branch, false_branch) -> (
    ...
    let integrateInst = 
      if List.length falseBranchInst = 0 then
        trueBranchInst @ [Label label1]
      else 
        trueBranchInst @ [Goto label2; Label label1] @ falseBranchInst @ [Label label2]
      in
...
let rec allocate_args args symbolMap instrList =
  ...
  | (varType, varName) :: nextArgs -> (
    let nextSM = SymbolMap.add varName (varName, ctyp_to_typ varType) symbolMap in
    allocate_args nextArgs nextSM instrList
  )
